view: catalog_product_available_sizes {
  derived_table: {
    sql: SELECT sku AS configurable_sku, count_sizes, count_sizes_in_stock, CAST(count_sizes_in_stock AS float) / count_sizes AS percentage_of_sizes_in_stock
      FROM (
        SELECT sku
  ...
  measure: percentage_of_sizes_in_stock {
    ...
    type: number
    sql: 100.00 * (CAST(${in_stock} AS float) / NULLIF(${total},0)) ;;
    value_format: "#.00\%"
  }
}
