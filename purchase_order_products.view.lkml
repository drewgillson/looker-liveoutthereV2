view: purchase_order_products {
  derived_table: {
    sql: SELECT po.po_date
      ...
      LEFT JOIN magento.purchase_supplier AS sup
        ON po.po_sup_num = sup.sup_id
      WHERE pop.pop_order_num IS NULL AND 7=7
       ;;
  ...
  dimension_group: predicted_invoice {
    type: time
    description: "Date the 1st invoice will probably arrive, based on terms"
    sql: CASE WHEN ISNUMERIC(${TABLE}.po_terms) = 1 AND ${TABLE}.po_status <> 'cancelled'
  ...
  dimension: status {
    ...
    case: {
      when: {
        sql: ${TABLE}.po_status = 'New' ;;
        label: "New"
      }

      when: {
        sql: ${TABLE}.po_status = 'acknowledged' ;;
        label: "Acknowledged"
      }

      when: {
        sql: ${TABLE}.po_status = 'waiting_for_delivery' ;;
        label: "Shipped"
      }

      when: {
        sql: ${TABLE}.po_status = 'waiting_to_receive' ;;
        label: "Arrived"
      }

      when: {
        sql: ${TABLE}.po_status = 'in_progress' ;;
        label: "In Progress"
      }

      when: {
        sql: ${TABLE}.po_status = 'in_progress_partial' ;;
        label: "Partial Receipt"
      }

      when: {
        sql: ${TABLE}.po_status = 'complete' ;;
        label: "Complete"
      }

      when: {
        sql: ${TABLE}.po_status = 'cancelled' ;;
        label: "Cancelled"
      }

      when: {
        sql: ${TABLE}.po_status = 'closed' ;;
        label: "Closed"
      }

      when: {
        sql: ${TABLE}.po_status = 'discrepancy' ;;
        label: "Unresolved Discrepancies"
      }

      when: {
        sql: ${TABLE}.po_status = 'waiting_for_supplier' ;;
        label: "Waiting for RA"
      }
