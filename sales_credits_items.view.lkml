view: sales_credits_items {
  derived_table: {
    sql: SELECT ROW_NUMBER() OVER (ORDER BY creditmemo_created) AS row,
            a.*
            , (refunded_qty * average_cost.value) AS extended_cost
            , (refunded_qty * msrp.price) AS refunded_msrp
          FROM (
            SELECT [type]
              , storefront
              , creditmemo_created
              ...
              , product_id
              , refunded_qty
              , refund_for_return - adjustment_tax_amount AS refund_for_return
              , refund_for_other_reason - adjustment_tax_amount AS refund_for_other_reason
              , refund_for_shipping - shipping_tax_amount AS refund_for_shipping
              , adjustment_tax_amount + shipping_tax_amount AS refunded_tax
              , ISNULL(refund_for_return,0) + ISNULL(refund_for_other_reason,0) + ISNULL(refund_for_shipping,0) AS refunded_total
              , mailed
              , mailed_description
              , refunded_to_giftcredit
              , tax_class_id
            FROM (
              ...
              , MAX(h.ot_created_at) AS mailed
              , MAX(h.ot_description) AS mailed_description
              ...
              LEFT JOIN (SELECT ot_entity_id, MIN(ot_created_at) AS ot_created_at, CAST(ot_description AS nvarchar(1024)) AS ot_description FROM magento.organizer_task WHERE ot_caption = 'Return item accepted at Post Office' GROUP BY ot_entity_id, CAST(ot_description AS nvarchar(1024))) AS h
            ...
            LEFT JOIN (SELECT ot_entity_id, MIN(ot_created_at) AS ot_created_at, CAST(ot_description AS nvarchar(1024)) AS ot_description FROM magento.organizer_task WHERE ot_caption = 'Return item accepted at Post Office' GROUP BY ot_entity_id, CAST(ot_description AS nvarchar(1024))) AS d
              ON b.order_id = d.ot_entity_id
            LEFT JOIN magento.sales_flat_creditmemo_comment AS e
              ON b.entity_id = e.parent_id
            LEFT JOIN magento.catalog_product_entity_int AS f
              ON a.product_id = f.entity_id AND f.attribute_id = (SELECT attribute_id FROM magento.eav_attribute WHERE attribute_code = 'tax_class_id' AND entity_type_id = 4)
            WHERE a.row_total > 0
            GROUP BY b.created_at, c.entity_id, b.increment_id, b.entity_id, CAST(e.comment AS varchar(255)), a.product_id, a.qty, a.row_total, a.discount_amount, b.entity_id, a.tax_amount, b.giftcard_refund_amount, f.value
            UNION ALL
            SELECT 'credit' AS [type]
